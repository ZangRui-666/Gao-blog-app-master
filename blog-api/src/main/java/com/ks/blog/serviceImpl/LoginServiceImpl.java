...
import com.ks.blog.utils.JWTUtils;
...
public class LoginServiceImpl implements LoginService {
    ...
    private static final String salt = "Kanfei1!#$";
    @Override
    public Result login(LoginParam loginParam) {
         ...
         * token放入redis当中 redis token:user信息关系 设置过期时间(登陆认证的时候 先认证token字符串是否合法, 去redis认证是否存在)
         *
         */
    ...
    public Result register(LoginParam loginParam) {
         ...
         * 4 生成token
         * 5 存入redis 并返回
         * 6 注意 加上事务 中间的任何过程出现问题 注册的用户需要回滚
         */
        String account = loginParam.getAccount();
        String password = loginParam.getPassword();
